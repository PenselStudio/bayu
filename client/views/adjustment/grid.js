Template.adjustGrid.adjusts = function(){
  if(Session.get('isGridFiltered')){
    //alert('filtered');
    var selector = Session.get('filterSelector');
    return Adjustments.find(selector, {sort : {regDate : -1}});
  } else {
    //alert('unfiltered');
    return Adjustments.find({},{sort : {regDate : -1}});
  }
}

Template.adjustGrid.years = function(){
  var startYear = 2015;
  var obj = {};
  var arr = [];
...
///////////////////////////Rendered
Template.adjustGrid.rendered = function(){
  Session.set('isGridFiltered',false);
